...
classType
    : CLASS (LPAREN identifier (COMMA identifier)* RPAREN)? SEMI
        classBlock
        END
    ;
...
methodDeclaration
    : (PROCEDURE | FUNCTION) identifier (formalParameterList)? (COLON resultType)? (SEMI directive)* SEMI
    ;
directive
    : identifier
    ;
...
constructorDefinitionPart
    : constructorDefinition SEMI
    ;

destructorDefinitionPart
    : destructorDefinition SEMI
    ;
...
compoundStatement
    : BEGIN localBlock END
    ;

localBlock
    : (variableDeclarationPart)? statements
    ;
...
readln
    : READLN (LPAREN readArguments RPAREN)?
    ;
...
readArg
    : IDENT              // reuse existing IDENT token
    ;

writeArg
    : STRING_LITERAL     // reuse existing STRING_LITERAL token
    | writeExpr
    ;

writeExpr
    : IDENT (DOT IDENT)*    // name, a, b
    | IDENT PLUS IDENT      // a + b
    ;
